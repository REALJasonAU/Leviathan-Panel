...
- API-key authentication accepts `Authorization: Bearer lvk_...`, stores only SHA-256 hashes, shows the raw key once on creation, tracks last use, supports expiry and revocation, and enforces scopes through the same permission gate used by Leviathan users. Legacy `vtk_...` keys are still accepted for compatibility.
...
- Scheduled task execution now goes through job records with pending/running/success/failed/cancelled states, lock fields, retry metadata, and a local worker. SQL-backed deployments store those records in the panel control-plane database.
...
curl -X POST "$API_URL/v1/api-keys" \
  -H "Cookie: leviathan_session=<session-cookie>" \
...
Local development uses the built-in job worker. Production deployments can keep SQL-backed job records for smaller installs, but high-throughput/multi-region installs should add a Redis/BullMQ adapter behind the same `store.createJob`, `store.claimPendingJobs`, and `store.updateJob` interface.
